...
class Train():
     ...
     def fit (self, net1, net2, trGen, cvGen, criterion1, criterion,optimizer,epoch,totalepoch,flag, num):
        net1.train()
        net2.train()
        ...
        if flag == True :  
          ...
          for i, data in enumerate(trGen,0):
            ...
            mse_loss = criterion1(x,tgt.detach())   ## get the mse loss for net1
           
            x = self.integrate(x.transpose(2,1),inputs.float().cuda()) ### do integration here            
            outputs= net2(x.cuda())              ## asr input 

            labels = labels.astype(numpy.int64)
            labels = torch.from_numpy(labels)
            labels = labels.long().cuda()
            loss   = criterion(outputs, labels.detach()) + mse_loss * 0.3 ## adding mse loss to the main loss	
            loss.backward()
            optimizer.step()
...
#corss validation step  
        ...
        for j, data in enumerate(cvGen,0):
         ...
         loss_cv = criterion(output, label.detach()) + mse_loss * 0.3 ## adding mse loss to the main loss 
         ...
         if j % cv_steps == (cv_steps - 1):
           ...
           if epoch == 0:
               ...
               torch.save(net2.state_dict(),exp + '/dnn_nnet_ASR.model'+ str(num))
               torch.save(net1.state_dict(),exp + '/dnn_nnet_ENV.model'+ str(num))
               print('Saved model')
               break
           #else:
           #  break
...
for epoch in range(learning['minEpoch']):
 ...
 abc=train1.fit(net1 , net2, trGen, cvGen,criterion1,criterion,optimizer,epoch,1,flag, num)
 num = num + 1
