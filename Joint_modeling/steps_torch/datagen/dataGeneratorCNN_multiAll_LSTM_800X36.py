...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
	...
	outputList =[]
	 ...
	 featMat_label = featMat_label[:,0:trim_lb]
	 ...
	 #bp()
 	 ...
 	 featMat_label_tp=numpy.empty((1,198))
	 for x in range(f):
		
			temp1=featListFinal_tp
			temp2=featMat_label_tp
			featListFinal_tp=numpy.concatenate((temp1,featListFinal[:,:,((x)*800):((x+1)*800),:]),axis=0)
			featMat_label_tp=numpy.concatenate((temp2,featMat_label[:,((x)*198):((x+1)*198)]),axis=0)
	 #bp()
	 featListFinal = featListFinal_tp[1:,:,:,:]
	 featMat_label = featMat_label_tp[1:,:]
	 #bp()
