...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
        ...
        while True:
            ...
            uid_tgt, featMat_tgt = kaldiIO.readUtterance(
                p3f.stdout)  # no deltas used

            if uid == None:
                return (numpy.transpose(numpy.expand_dims(numpy.concatenate((featList), axis=2), axis=1), (3, 1, 0, 2)), numpy.squeeze(numpy.concatenate((labelList), axis=2).T, axis=1), numpy.transpose(numpy.expand_dims(numpy.concatenate((featList_tgt), axis=2), axis=1), (3, 1, 0, 2)))
            #################################### change made here for sliding window ####################################################
            # featMat=numpy.random.rand(3078,36)
            if featMat_tgt.shape[0] < 800:
                continue
            f = featMat_tgt.shape[0]//800
            add_l = numpy.repeat(featMat[0:1, :], 40, axis=0)
            add_r = numpy.repeat(featMat[featMat.shape[0]-1:featMat.shape[0], :], 40, axis=0)

            add_l_tgt = numpy.repeat(featMat_tgt[0:1, :], 40, axis=0)
            add_r_tgt = numpy.repeat(featMat_tgt[featMat.shape[0]-1:featMat.shape[0], :], 40, axis=0)

            featMat_test = numpy.concatenate((add_l, featMat, add_r), axis=0)
            splitter_test, add_sample_test = buffer_2d_new(
                featMat_test, 880, 80, 'nodelay')

            featMat_test_tgt = numpy.concatenate(
                (add_l_tgt, featMat_tgt, add_r_tgt), axis=0)
            splitter_test_tgt, add_sample_test_tgt = buffer_2d_new(
                featMat_test_tgt, 880, 80, 'nodelay')

            if uid not in labels:
                continue
            label = labels[uid]
            label, add_sample_l = buffer_2d(
                numpy.expand_dims(label, axis=1), 198, 0, 'nodelay')
            featList.append(splitter_test[:, :, 0:f])
            featList_tgt.append(splitter_test_tgt[:, :, 0:f])
    ...
    def __next__(self):
        ...
        zMini = self.z[self.batchPointer:self.batchPointer+self.batchSize]
        self.batchPointer += self.batchSize
        # bp()
        return (xMini, yMini, zMini)
