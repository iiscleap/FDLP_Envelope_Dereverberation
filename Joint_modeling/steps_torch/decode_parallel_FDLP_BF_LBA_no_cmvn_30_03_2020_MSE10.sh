...
#   echo "apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata2/${JOB}/utt2spk scp:$sdata2/${JOB}/cmvn.scp scp:$sdata2/${JOB}/feats.scp ark:- |  splice-feats $splice_opts ark:- ark,scp:$sdataF/${JOB}/feats3.ark,$sdataF/${JOB}/feats3.scp" >> ${dir}/log/decode.${JOB}.sh
   ...
   echo "feats=\"ark,s,cs:$cpython steps_torch/nnet-forward_joint_training_cpu_no_cmvn_30_03_2020_MSE10.py $srcdir/dnn_nnet_ENV.model17 $srcdir/dnn_nnet_ASR.model17 $srcdir/dnn.priors.csv $data $nj $JOB  |\""  >> ${dir}/log/decode.${JOB}.sh 
  ...
  qsub -l hostname=compute-0-[0-4] -N ${basedir}_env -e ${curPwd}/${dir}/log/decode.${JOB}.log -o ${curPwd}/${dir}/log/decode.${JOB}.log -S /bin/bash ${curPwd}/${dir}/log/decode.${JOB}.sh   #### change here for gpu
  sleep 5s
done
sleep 10s;
