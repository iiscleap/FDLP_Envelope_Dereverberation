...
def integrate(y,inp):
        ...
        filters = torch.from_numpy(wind).float().cpu()  #changed from .cuda() to .cpu() for decoding by anurenjan
        ...
        abcd_no_mean=abcd-abcd_mean.repeat(1,1,abcd.shape[2],1)
        #bp()
        #1,1,T,36
    ...
    while uttId:
	 ...
	 for i in range(f-1):
		...
		mpy=net1(torch.from_numpy(ipt[i:i+1,:,:,:]).float())
                mpy=integrate(mpy.transpose(2,1),torch.from_numpy(ipt[i:i+1,:,:,:]).float())
                mpy=net2(mpy)
		mpy_full.append(mpy.detach().numpy())
	 mpy_last=net1(torch.from_numpy(last_chunk).float())
	 mpy_last=integrate(mpy_last.transpose(2,1),torch.from_numpy(last_chunk).float())
