...
basedir="ENV_decode"        #`echo $dnndir | xargs -n1 basename | awk -F'_' '{print $1}'`
#mkdir -p $sdataF
...
#   echo "apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata2/${JOB}/utt2spk scp:$sdata2/${JOB}/cmvn.scp scp:$sdata2/${JOB}/feats.scp ark:- |  splice-feats $splice_opts ark:- ark,scp:$sdataF/${JOB}/feats3.ark,$sdataF/${JOB}/feats3.scp" >> ${dir}/log/decode.${JOB}.sh
   ...
   echo "feats=\"ark,s,cs: python steps_torch/nnet-forward_FDLP_cnn_BF_lstm_BatchNorm_Adam.py $srcdir/dnn_nnet.model $srcdir/dnn.priors.csv $data $nj $JOB  |\""  >> ${dir}/log/decode.${JOB}.sh 
  ...
  qsub -q med.q -N ${basedir} -e ${curPwd}/${dir}/log/decode.${JOB}.log -o ${curPwd}/${dir}/log/decode.${JOB}.log -S /bin/bash ${curPwd}/${dir}/log/decode.${JOB}.sh
  sleep 5s
done
sleep 10s;
