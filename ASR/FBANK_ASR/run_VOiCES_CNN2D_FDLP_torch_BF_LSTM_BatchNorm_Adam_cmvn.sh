...
exp=exp/torch_FDLP_2DCNN_wpe_baseline_noCMVN
mkdir -p $exp
...
cp ${ali}/final.mdl ${ali}/tree $exp/
 bash steps_torch/decode_parallel_FDLP_cnn_BF_lstm_BatchNorm_Adam_cmvn.sh --nj $nj \
    --add-deltas "false" --norm-vars "true" --splice-opts "--left-context=10 --right-context=10" \
    $test ${graphdir} $exp $exp/test_tri4b_100_reverb/${testdir}/decode
