...
class dataGeneratorFBANKCNN_multiAll:
    def __init__ (self, data, ali, exp, batchSize=256, spliceSize=21):
        ...
        #from backports import tempfile
        #with tempfile.TemporaryDirectory() as temp_dir:
        #  self.labelDir = temp_dir

#        print (self.labelDir.dtype)
        ...
        self.inputFeatDim = 40 ## IMPORTANT: HARDCODED. Change if necessary.
        self.outputFeatDim = self.readOutputFeatDim()
        self.splitDataCounter = 0
        
        self.x = numpy.empty ((0,1,8, self.spliceSize, self.inputFeatDim), dtype=numpy.float32)
...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
        ...
        while True:
            ...
            if uid in labels:
                #abc=featMat.reshape(featMat.shape[0],1,self.spliceSize,self.inputFeatDim)
                #print(abc[0,0,0,0:35])
                ...
                featListFourth = numpy.concatenate((featListThird,featMat4.reshape(featMat4.shape[0],1,self.spliceSize,self.inputFeatDim)),axis=1) 
