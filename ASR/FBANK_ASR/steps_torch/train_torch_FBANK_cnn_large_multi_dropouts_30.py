...
from dataGeneratorFBANKCNN_multiAll import dataGeneratorFBANKCNN_multiAll
import numpy
import sys
from Net_dropouts_30 import Net
...
trGen = dataGeneratorFBANKCNN_multiAll (data_tr, ali_tr, gmm, learning['batchSize'],learning['spliceSize'])
cvGen = dataGeneratorFBANKCNN_multiAll (data_cv, ali_cv, gmm, learning['batchSize'],learning['spliceSize'])
     ...
     def fit (self, net, trGen, cvGen,criterion,optimizer,epoch,totalepoch):
        ...
        for i, data in enumerate(trGen,0):
         ...
         if i % trGen.numSteps ==(trGen.numSteps - 1):
          ...
          print ('TRAIN ==> [%d,%5d] LOSS: %.3f  ACCURACY  : %.3f %% '%(epoch+1,i+1,running_loss_tr/trGen.numSteps,100 * correct_tr / total_tr))
          running_loss_tr =0.0
          break
#corss validation step   
        ...
        for j, data in enumerate(cvGen,0):
         ...
         if j % cvGen.numSteps == (cvGen.numSteps - 1):
           ...
           print ('VALID ==> [%d,%5d] LOSS_CV: %.3f  ACCURACY_CV  : %.3f %%'%(epoch+1,j+1,running_loss_cv/cvGen.numSteps,100 * correct_cv / total_cv))
           running_loss_cv =0.0
